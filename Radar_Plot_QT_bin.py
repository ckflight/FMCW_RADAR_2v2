...
with open(BIN_FILE, "rb") as record_file:
    file_bytes = record_file.read()
...
CHIRP_NUMBER = read_u16_be(info, idx); idx += 2
...
date_bytes = info[64:128].split(b"\x00")[0]
RECORD_DATE = date_bytes.decode("ascii", errors="ignore")
...
adc_u16 = np.frombuffer(raw_data, dtype=np.dtype(BIN_SAMPLE_DTYPE))

usable_samples = (len(adc_u16) // NUMBER_OF_SAMPLES) * NUMBER_OF_SAMPLES
adc_u16 = adc_u16[:usable_samples]

chirps_u16 = adc_u16.reshape(-1, NUMBER_OF_SAMPLES)

ADC_MASK = (1 << ADC_RESOLUTION) - 1
chirps_u16 = chirps_u16 & ADC_MASK

chirps_float = (chirps_u16.astype(np.float32) / float(1 << ADC_RESOLUTION)) * 3.3

RECORD_COUNTER = len(chirps_u16)
print("BIN chirps loaded:", RECORD_COUNTER)
